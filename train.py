...
def main_worker(local_rank, world_size, rank_offset, config):
    ...
    dist.init_process_group(
        backend='nccl', init_method=config.url, world_size=world_size, rank=rank)
    # if rank==0:
    #     try:
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=config.batch_size, sampler=train_sampler, pin_memory=True, num_workers=config.workers)
    validate_loader = torch.utils.data.DataLoader(
        validate_dataset, batch_size=config.batch_size, sampler=validate_sampler, pin_memory=True, num_workers=config.workers)
    logs = {}
    start_epoch = 0
    ...
    ), lr=config.learning_rate, betas=config.adam_betas, weight_decay=config.weight_decay)
    ...
    for epoch in range(start_epoch, config.epochs):
        ...
        run(logs=logs, data_loader=train_loader, net=net, optimizer=optimizer,
            local_rank=local_rank, config=config, AG=AG, phase='train')
        run(logs=logs, data_loader=validate_loader, net=net, optimizer=optimizer,
            local_rank=local_rank, config=config, phase='valid')
...
def train_loss(loss_pack, config):
    ...
    loss = config.ensemble_loss_weight * \
        loss_pack['ensemble_loss']+config.aux_loss_weight*loss_pack['aux_loss']
    if config.AGDA_loss_weight != 0:
        loss += config.AGDA_loss_weight * \
            loss_pack['AGDA_ensemble_loss'] + \
            config.match_loss_weight*loss_pack['match_loss']
    return loss
...
def run(logs, data_loader, net, optimizer, local_rank, config, AG=None, phase='train'):
    ...
    if local_rank == 0:
        logging.info('{}: {}, Time {:3.2f}'.format(
            phase, '  '.join(batch_info), end_time - start_time))
...
def distributed_train(config, world_size=0, num_gpus=0, rank_offset=0):
    ...
    if not world_size:
        world_size = num_gpus
    mp.spawn(main_worker, nprocs=num_gpus, args=(
        world_size, rank_offset, config))
    torch.cuda.empty_cache()
