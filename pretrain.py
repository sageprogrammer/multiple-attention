...
def main_worker(local_rank, world_size, rank_offset, config):
    ...
    dist.init_process_group(backend='nccl', init_method='file://%s/.pytorch_distribute' %
                            os.getcwd(), world_size=world_size, rank=rank)
    if rank == 0:
        try:
    ...
    logs = {}
    start_epoch = 0
    net = netrunc(config.net, config.feature_layer,
                  config.num_classes, config.dropout_rate, config.pretrained)
    ...
    for epoch in range(start_epoch, config.epochs):
        ...
        run(logs=logs, data_loader=train_loader, net=net, optimizer=optimizer,
            local_rank=local_rank, config=config, phase='train')
