...
class train_config:
    def __init__(self, name, recipes=[], **params):
        ...
        self.AGDA_config = dict(kernel_size=self.AGDA_kernel_size, dilation=self.AGDA_dilation, sigma=self.AGDA_sigma, scale_factor=self.AGDA_scale_factor,
                                threshold=self.AGDA_threshold, zoom=self.AGDA_zoom, noise_rate=self.AGDA_noise_rate, mode=self.AGDA_mode)
    ...
    def mkdirs(self):
        ...
        os.system(
            'git add . && git commit -m "{}" && git tag {} -f'.format(self.comment, self.name))
