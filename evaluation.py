...
def test_eval(net, setting, testset):
    ...
    test_loader = torch.utils.data.DataLoader(
        test_dataset, batch_size=setting['imgs_per_video'], shuffle=False, pin_memory=True, num_workers=8)
...
def dfdc_metric(testset):
    ...
    rs['logloss'] = torch.nn.functional.cross_entropy(
        video_preds, video_labels).item()
    return rs
...
def all_eval(name, ckpt=None, test_sets=['ff-all', 'celeb', 'deeper']):
    ...
    state_dict = torch.load('checkpoints/%s/ckpt_%s.pth' %
                            (name, ckpt))['state_dict']
    ...
    if 'deeper' in test_sets:
        ...
        testset = deeperforensics_dataset(
            'test')+FF_dataset('Origin', codec, 'test')
        test_eval(net, setting, testset)
...
def eval_meancorr(name, ckpt=None):
    ...
    test_dataset = DeepfakeDataset(phase='test', **setting)
    test_loader = torch.utils.data.DataLoader(
        test_dataset, batch_size=30, shuffle=False, pin_memory=True, num_workers=8)
    count = 0
    mc_count = 0
    for i, (X, y) in enumerate(test_loader):
        x = X.to('cuda', non_blocking=True)
        with torch.no_grad():
            ...
            for a in range(0, config.num_attentions-1):
                for b in range(a+1, config.num_attentions):
                    mc_count += torch.sum(torch.sum(
                        attention_maps[:, a, :]*attention_maps[:, b, :], dim=-1)/(srs[:, a]*srs[:, b]))
