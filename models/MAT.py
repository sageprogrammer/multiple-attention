...
class Texture_Enhance_v1(nn.Module):
    ...
    def forward(self, feature_maps, attention_maps=(1, 1)):
        ...
        feature_maps = self.bn_last(self.conv_last(
            F.relu(self.bn4(feature_maps3_), inplace=True)))
    ...
    def forward(self, feature_maps, attention_maps=(1, 1)):
        ...
        if feature_maps.size(2) > feature_maps_d.size(2):
            feature_maps = feature_maps - \
                F.interpolate(
                    feature_maps_d, (feature_maps.shape[2], feature_maps.shape[3]), mode='nearest')
        attention_maps = (torch.tanh(F.interpolate(attention_maps.detach(
        ), (H, W), mode='bilinear', align_corners=True))).unsqueeze(2) if type(attention_maps) != tuple else 1
        ...
        for j in range(M):
            for k in range(j+1, M):
                inter_class_loss += F.relu(self.margin -
                                           torch.dist(fcts[j], fcts[k]), inplace=False)
...
class Auxiliary_Loss_v1(nn.Module):
    ...
    def forward(self, feature_map_d, attentions, y):
        ...
        intra_class_loss = F.relu(torch.norm(
            feature_matrix-fcts, dim=-1)-inner_margin)
        intra_class_loss = torch.mean(intra_class_loss)
        inter_class_loss = 0
        for j in range(M):
            for k in range(j+1, M):
...
class netrunc(nn.Module):
    def __init__(self, net='xception', feature_layer='b3', num_classes=2, dropout_rate=0.5, pretrained=False):
        ...
        if 'xception' in net:
            ...
            self.net = EfficientNet.from_pretrained(
                net, advprop=True, num_classes=num_classes, escape=feature_layer)
        self.feature_layer = feature_layer
        with torch.no_grad():
