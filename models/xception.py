...
class xception(nn.Module):
    def __init__(self, num_classes=2, pretrained='imagenet', escape=''):
        ...
        self.seq = []
        self.seq.append(('b0', [self.conv1, lambda x:self.bn1(
            x), self.relu1, self.conv2, lambda x:self.bn2(x)]))
        ...
        self.seq.append(('final', [self.conv3, lambda x:self.bn3(
            x), self.relu3, self.conv4, lambda x:self.bn4(x)]))
        self.seq.append(('logits', [self.relu4, lambda x:F.adaptive_avg_pool2d(
            x, (1, 1)), lambda x:x.view(x.size(0), -1), self.last_linear]))
        if pretrained == 'imagenet':
            self.load_state_dict(model_zoo.load_url(
                'http://data.lip6.fr/cadene/pretrainedmodels/xception-43020ad28.pth'), strict=False)
        elif pretrained:
